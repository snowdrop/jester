...
- Write the test scenario once and run it everywhere (Kubernetes, bare metal, etc.)
- Developer and Test friendly
- Test isolation: ephemeral namespace, configuration, ...

Concepts:
- `Scenario` - entry point for the jCloud JUnit 5 extension
- `Services` - how to use services in your tests
- `Managed Resource` - how resources are going to be deployed in the target scenario? The framework will locate the right managed resource and attach it to the service. This is transparently done for users. 
...
And add the scenario that will run the above image and map the container port 8080 to a local port:
...
And now, let's write our first scenario:
...
In the previous example, we have created our first scenario using the test framework, configured the Failsafe Maven plugin, and execute our tests on Native. Let's now create a scenario with multiple Quarkus instances.
...
The same scenario works locally and in Kubernetes.
...
The service is the point of contact between the test/scenario and the managed resource, regardless of the environment where the resource is running. Example:
...
For (1) annotations, we can annotate the scenario using `@ServiceConfiguration` (for common properties):
...
**Note**: The `<SERVICE NAME>` is the name of the service within the scenario. For example, in the following example, the `<SERVICE NAME>` value is `app`.
...
- `io.jcloud.configuration` - configure the services and scenario configuration

...
public abstract class AbstractSqlDatabaseIT {
    ...
    public void createBadPayload() {
        BookReactive bookReactive = new BookReactive();
        bookReactive.id = Long.valueOf(INVALID_ID);
        bookReactive.title = "foo";
        bookReactive.author = "bar";

        given().contentType(ContentType.JSON).body(bookReactive).post("/book").then()
    ...
    public void update() {
        BookReactive bookReactive = new BookReactive();
        bookReactive.id = Long.valueOf(VALID_ID);
        bookReactive.title = "Schismatrix";
        bookReactive.author = "Bruce Sterling";

        given().contentType(ContentType.JSON).body(bookReactive).put("/book/8").then().statusCode(HttpStatus.SC_OK)
    ...
    public void updateWithUnknownId() {
        ...
        given().contentType(ContentType.JSON).body(bookReactive).put("/book/999").then()
    ...
    public void updateBadPayload() {
        ...
        given().contentType(ContentType.JSON).body(bookReactive).put("/book/8").then()
                .statusCode(HttpStatus.SC_UNPROCESSABLE_ENTITY)
                .body("code", equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY))
